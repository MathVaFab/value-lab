function graphListener (evt, item) {
  item.forEach((chartElement) => {
    if (chartElement.datasetIndex === 0) {
      ...
      const legendTarget = document.querySelector(`.target[data-target="${index}"]`)
      const button = legendTarget.querySelector('.accordion')
      if (!legendTarget.classList.contains('show')) showTargetLegend({ currentTarget: button })
    }
  })
}
...
function generateLegend (landbotData) {
  ...
  landbotData.forEach((target, index) => {
    ...
    target.pillars.forEach((pillar) => {
      ...
      clonePillars.querySelector('.target-pillar-name').textContent = pillar.name
      clonePillars.querySelector('.target-pillar-def').textContent += pillar.def
      clonePillars.querySelector('.target-pillar-kpis').textContent += pillar.kpis.join(', ')
        ...
        datasets: [
          ...
          },
          {
            ...
            datalabels: {
              ...
              rotation: (ctx) => {
                let angle = chartPieceAngle / 2 + ctx.dataIndex * chartPieceAngle - 90
                if (ctx.dataIndex >= ctx.dataset.name.length / 2) {
                  angle = chartPieceAngle / 2 + (ctx.dataIndex - ctx.dataset.name.length / 2) * chartPieceAngle - 90
                }
                return angle
              }
            }
